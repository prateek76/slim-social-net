...
{% block content %}
	...
	<div class="post_parent">
		<div class="post_head">post by <a style="color: #000;" href="{{ urlFor('user.profile', {username: post.post_username}) }}">{{ post.post_username }}</a><span style="font-size: 11px;
		...
		{% if auth %}
			...
			<div style="height: 30px;padding: 10px;padding-bottom: 27px;background-color: #a7d6ff7a"><span>{{ random(200) }} likes</span>&nbsp<a href="#"><span class="glyphicon glyphicon-thumbs-up"></span></a>&nbsp<a href="#"><span class="glyphicon glyphicon-thumbs-down"></span></a></div><br>
			<div style="height: 40px;background-color: #fff;border-radius: 50px;">
				<form action="{{ urlFor('comment.post') }}" method="post">
					...
					<span><button style="	position: absolute;
										    ...
										    width: 117px;
										    height: 41px;
										    outline: none;" class="btn btn-primary" type="submit"><span style="font-size: 25px" class="glyphicon glyphicon-send"></span></button></span>
					<input type="hidden" name="{{ csrf_key }}" value="{{ csrf_token }}">
		...
		<div style="background: #0000001a;
			...
			<div style="background: #fff;
					    border-radius: 50px;
					    padding: 20px;
    					word-wrap: break-word;" class="comment"><span><a href="{{ urlFor('user.profile', {username: comment.post_username}) }}">{{ comment.post_username }} </a></span><span>{{ comment.post }}</span><span style="font-size: 11px;
